...
def _apply_legacy_compat(config: dict) -> None:
    legacy_keywords = config.get("daily_paper_keywords")
    daily = config.setdefault("daily_papers", {})
    default_keywords = DEFAULT_CONFIG["daily_papers"]["keywords"]
    current_keywords = daily.get("keywords")
    if legacy_keywords and (
        not current_keywords
        or current_keywords == default_keywords
    ):
        daily["keywords"] = list(legacy_keywords)

    paths = config.setdefault("paths", {})
    if not paths.get("paper_notes_folder"):
...
def research_fields_dir() -> Path:
    return obsidian_vault_path() / paths_config()["research_fields_folder"]
...
def research_fields_config() -> dict:
    return load_user_config().get("research_fields", {})
