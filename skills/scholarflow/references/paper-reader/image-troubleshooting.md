...
```bash
wget -O /tmp/{MethodName}.pdf "https://arxiv.org/pdf/{arxiv_id}.pdf"
mkdir -p {笔记所在目录}/pngs
pdfimages -png /tmp/{MethodName}.pdf {笔记所在目录}/pngs/img
```
...
```bash
python3 scripts/daily-papers/download_note_images.py "{笔记路径}"
```
